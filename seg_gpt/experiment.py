...
from mess.datasets.TorchvisionDataset import (TorchvisionDataset,
                                              get_detectron2_datasets)

# NUMBER_OF_SHOTS = [i for i in range(1, 8)] + [16, 32]
NUMBER_OF_SHOTS = [1, 2, 3, 4]
CLASSES_TO_IGNORE = [
    "background",
    "others",
    "unlabeled",
...
if __name__ == "__main__":
    ...
    if not parser.parse_args().use_existent_mappings:
        ...
        test_class_img_mapping = np.load("test_class_img_mapping.npy", allow_pickle=True).item()


    # Iterate over all available classes
    metrics_per_class = {}
    for class_ind in tqdm.tqdm(class_img_mapping.keys()):
    ...
    # Save metrics
    np.save("metrics.npy", metrics_per_class)
