...
def handle_dstdir(dstdir, reset):
    dstdir = Path(dstdir)
    try:
...
def main(precomputed_dir, dstdir, max_clicks_per_image=10, reset=False, ds=None):
    ...
    dstdir = handle_dstdir(dstdir, reset)
    if dstdir is None:
        return
    (dstdir / 'vis').mkdir()
    ...
    for ds_name in ds_names:
        ...
        for sample_ind, sample in enumerate(ds):
            ...
            for mvalue_ind, value in enumerate(np.unique(mask)):
                ...
                if gt_masks[0].sum() == 0:
                    metrics_per_ds[sample_ind][class_name] = None
                    continue
